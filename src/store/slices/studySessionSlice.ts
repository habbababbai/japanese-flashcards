...
import { StudySession, StudyProgress, Kana } from "../../types";
    ...
    reducers: {
        ...
        endSession: (
            ...
            if (state.currentSession) {
                ...
                const incorrectCount = progress.filter(
                    (p) => !p.isCorrect
                ).length;
        ...
        addProgress: (state, action: PayloadAction<StudyProgress>) => {
            ...
            state.kanaProgress[progress.kanaId].lastReviewed =
                progress.timestamp;
        },
...
export const selectSessionsByType = (
    ...
    state.studySession.sessions.filter(
        (session) => session.kanaType === kanaType
    );
